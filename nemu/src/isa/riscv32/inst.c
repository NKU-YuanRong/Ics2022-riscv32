...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, gpr(dest) = *CSR(imm); *CSR(imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs	 , I, t = *CSR(imm); *CSR(imm) = t | src1; gpr(dest) = t);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, t = *CSR(imm); *CSR(imm) = src1; gpr(dest) = t);
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = cpu.sr.mepc + 4);
