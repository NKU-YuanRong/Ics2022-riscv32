...
INSTPAT_START();
	...
	// INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = (src1 + imm);if(s->isa.inst.val==0x00008067)stack_return(s->pc, s->dnpc);else stack_call(s->pc, s->dnpc););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4; s->dnpc = src1 + imm);
	...
	// INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, csrrwrs(dest, src1, imm, true);IFDEF(CONFIG_ETRACE, Log("etrace: csrrs")));
	// INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs	 , I, csrrwrs(dest, src1, imm, false);IFDEF(CONFIG_ETRACE, Log("etrace: csrrs")));
  ...
  // INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + imm;stack_call(s->pc, s->dnpc));
  ...
  // INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << imm);
  // INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> imm);
  // INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int32_t)src1 >> (int32_t)imm);
  // INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs	 , I, *CSR(imm) = (gpr(dest) = *CSR(imm)) | src1);
  // INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall	 , I, s->dnpc = isa_raise_intr(cpu.gpr[17], s->pc));
  ...
  // INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  // INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((long long)src1 * (long long)src2) >> 32);
  // // INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu   , R, R(dest) = (int32_t)((SEXT((long long)src1, 32) * SEXT((long long)src2, 32)) >> 32));
  ...
  // INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(dest) = src1 ^ src2);
  // INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = src1 >> src2);
  // INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (int32_t)src1 >> (int32_t)src2);
  // INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  // INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
